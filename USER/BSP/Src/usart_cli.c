...
uint8_t cli_rx_buffer[RX_BUFFSIZE] = {0};
uint8_t rx_flag = 0;
uint32_t rx_len = 0;
	...
	while(1) {
		if (rx_flag) {
			...
			if (rx_len == 3) {
				...
				control_key.buff[0] = cli_rx_buffer[2];
				control_key.buff[1] = cli_rx_buffer[1];
				control_key.buff[2] = cli_rx_buffer[0];
