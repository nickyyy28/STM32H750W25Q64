...
{
  /* USER CODE BEGIN 3 */
  *block_num  = w25qxx.SectorCount;
  *block_size = w25qxx.SectorSize;
  return (USBD_OK);
  /* USER CODE END 3 */
}
  ...
  /* USER CODE BEGIN 6 */
	if(lun == 0)
	{
		...
		for(i = 0; i < blk_len; i ++)
		{
			...
			W25qxx_ReadSector(buf + i * 4096, blk_addr + i, 0, 4096);
		}	
	}
	else if(lun ==1)
	{
		...
		for(i = 0; i < blk_len; i ++)
		{
			...
			//W25qxx_WriteSector((uint8_t *)(buff + 4096 * i), sector + i, 0, 4096);
			W25qxx_ReadSector(buf + i * 4096, blk_addr + i, 0, 4096);
		}	
	}
  return (USBD_OK);
  ...
  /* USER CODE BEGIN 7 */
	if(lun == 0)
	{
		...
		for(i = 0; i < blk_len; i ++)
		{
			...
			W25qxx_WriteSector((uint8_t *)(buf + 4096 * i), blk_addr + i, 0, 4096);
		}
	}
	else if(lun ==1)
	{
